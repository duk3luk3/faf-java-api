...
import com.faforever.integration.factories.NameFactory;
...
public class JsonApiNameIntegrationTest {
  ...
  public void getNames() {
    ...
    Player player = PlayerFactory.createPlayer("UseFactory", database);
    Name entity = NameFactory.builder().name("MyFirstName").player(player).database(database).build();

    assertEquals(1, database.getNameRepository().count());
    MockMvcHelper.of(this.mvc).perform(get("/data/name"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.data", hasSize(1)))
      .andExpect(jsonPath("$.data[0].id", is(String.valueOf(entity.getChangeTime()))))
      .andExpect(jsonPath("$.data[0].type", is("name")))
